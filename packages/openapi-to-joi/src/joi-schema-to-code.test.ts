...
describe('joiSchemaToCode()', function () {
  describe('any schema', function () {
    describe('allow()', function () {
      it.each([
        ...
        [Joi.any().allow('allow1', 'allow2'), 'Joi.any().allow("allow1","allow2")'],
        [Joi.any().allow(3), 'Joi.any().allow(3)'],
        [Joi.any().allow(3, 4), 'Joi.any().allow(3,4)'],
        [Joi.any().valid('valid1'), 'Joi.any().allow("valid1").only()'],
        [Joi.any().valid('valid1', 'valid2'), 'Joi.any().allow("valid1","valid2").only()'],
    ...
    describe('description()', function () {
      it.each([[Joi.any().description('description'), 'Joi.any().description("description")']])(
        'returns expected value',
        (schema, expected) => {
          expect(joiSchemaToCode(schema)).toEqual(expected)
        }
    ...
    describe('example()', function () {
      it.each([
        ...
        [Joi.any().example('example', { override: true }), 'Joi.any().example("example")'],
      ])('returns expected value', (schema, expected) => {
        expect(joiSchemaToCode(schema)).toEqual(expected)
      })
    })
    ...
    describe('invalid()', function () {
      it.each([
        ...
        [Joi.any().invalid('invalid1', 'invalid2'), 'Joi.any().invalid("invalid1","invalid2")'],
    ...
    describe('only()', function () {
      it.each([
        ...
        [Joi.any().allow('allow1', 'allow2').only(), 'Joi.any().allow("allow1","allow2").only()'],
        [Joi.any().valid('valid1', 'valid2').only(), 'Joi.any().allow("valid1","valid2").only()'],
      ])('returns expected value', (schema, expected) => {
        expect(joiSchemaToCode(schema)).toEqual(expected)
      })
    })
    ...
    describe('unit()', function () {
      it.each([[Joi.any().unit('unit'), 'Joi.any().unit("unit")']])('returns expected value', (schema, expected) => {
        expect(joiSchemaToCode(schema)).toEqual(expected)
      })
    })
  })
  ...
  describe('array schema', function () {
    it.each([
      ...
      [Joi.array().description('description'), 'Joi.array().description("description")'],
      ...
      [Joi.array().ordered(Joi.string(), Joi.number()), 'Joi.array().ordered(Joi.string(),Joi.number())'],
  ...
  describe('binary schema', function () {
    it.each([
      [Joi.binary().allow('allow'), 'Joi.binary().allow("allow")'],
      [Joi.binary().default('default'), 'Joi.binary().default("default")'],
      [Joi.binary().description('description'), 'Joi.binary().description("description")'],
  ...
  describe('boolean schema', function () {
    it.each([
      ...
      [Joi.boolean().description('description'), 'Joi.boolean().description("description")'],
  ...
  describe('date schema', function () {
    it.each([
      ...
      [Joi.date().description('description'), 'Joi.date().description("description")'],
      [Joi.date().example('example'), 'Joi.date().example("example")'],
      [Joi.date().forbidden(), 'Joi.date().forbidden()'],
      [Joi.date().greater('2022-01-01'), 'Joi.date().greater("2022-01-01T00:00:00.000Z")'],
      [Joi.date().invalid('invalid'), 'Joi.date().invalid("invalid")'],
      [Joi.date().iso(), 'Joi.date().iso()'],
      [Joi.date().label('label'), 'Joi.date().label("label")'],
      [Joi.date().less('2022-01-01'), 'Joi.date().less("2022-01-01T00:00:00.000Z")'],
      [Joi.date().max('2022-01-01'), 'Joi.date().max("2022-01-01T00:00:00.000Z")'],
      [Joi.date().min('2022-01-01'), 'Joi.date().min("2022-01-01T00:00:00.000Z")'],
  ...
  describe('number schema', function () {
    it.each([
      ...
      [Joi.number().description('description'), 'Joi.number().description("description")'],
  ...
  describe('object schema', function () {
    it.each([
      ...
      [Joi.object().description('description'), 'Joi.object({}).description("description")'],
      ...
      [Joi.object().keys({ a: Joi.string(), b: Joi.number() }), 'Joi.object({"a": Joi.string(),"b": Joi.number()})'],
  ...
  describe('string schema', function () {
    it.each([
      ...
      [Joi.string().base64(), 'Joi.string().base64({"urlSafe":false,"paddingRequired":true})'],
      [
      ...
      [Joi.string().dataUri(), 'Joi.string().dataUri({"paddingRequired":true})'],
      [Joi.string().dataUri({ paddingRequired: true }), 'Joi.string().dataUri({"paddingRequired":true})'],
      [Joi.string().default('default'), 'Joi.string().default("default")'],
      [Joi.string().description('description'), 'Joi.string().description("description")'],
      ...
      [Joi.string().hex({ byteAligned: true }), 'Joi.string().hex({"byteAligned":true})'],
      ...
      [Joi.string().replace(/abc/i, 'replacement'), 'Joi.string().replace(/abc/i, "replacement")'],

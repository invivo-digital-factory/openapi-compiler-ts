...
const appendAllow = (parts: string[], description: Description) => {
  ...
  if (values !== undefined) {
    parts.push(`.allow(${values.map((value) => JSON.stringify(value)).join(',')})`)
  }
}

const appendAlphanum = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'alphanum')
  if (rule !== undefined) {
    parts.push('.alphanum()')
  }
}
...
const appendAnd = (parts: string[], description: Description) => {
  const dependency = (((description as any).dependencies ?? []) as DependencyType[]).find((it) => it.rel === 'and')
  if (dependency !== undefined) {
    parts.push(`.and(${dependency.peers.map((it) => JSON.stringify(it)).join(',')})`)
  }
}

const appendNonStringLength = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'length')
  if (rule !== undefined) {
...
const appendBase64 = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'base64')
  if (rule !== undefined) {
...
const appendCase = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'case')
...
const appendCreditCard = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'creditCard')
  if (rule !== undefined) {
...
const appendDataUri = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'dataUri')
  if (rule !== undefined) {
...
const appendDateGreater = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'greater')
  if (rule !== undefined) {
...
const appendDateLess = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'less')
  if (rule !== undefined) {
...
const appendDateMax = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'max')
  if (rule !== undefined) {
...
const appendDateMin = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'min')
  if (rule !== undefined) {
...
const appendDomain = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'domain')
  if (rule !== undefined) {
...
const appendEmail = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'email')
  if (rule !== undefined) {
...
const appendFalsy = (parts: string[], description: Description) => {
  ...
  if (values !== undefined) {
    parts.push(`.falsy(${values.map((value) => JSON.stringify(value)).join(',')})`)
  }
}
...
const appendGuid = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'guid')
  if (rule !== undefined) {
...
const appendHex = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'hex')
  if (rule !== undefined) {
...
const appendHostname = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'hostname')
  if (rule !== undefined) {
    parts.push('.hostname()')
  }
}
...
const appendInteger = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'integer')
  if (rule !== undefined) {
    parts.push('.integer()')
  }
}
...
const appendInvalid = (parts: string[], description: Description) => {
  ...
  if (values !== undefined) {
    parts.push(`.invalid(${values.map((value) => JSON.stringify(value)).join(',')})`)
  }
}

const appendIp = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'ip')
  if (rule !== undefined) {
...
const appendIsoDate = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'isoDate')
  if (rule !== undefined) {
    parts.push('.isoDate()')
  }
}
...
const appendIsoDuration = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'isoDuration')
  if (rule !== undefined) {
...
const appendMatch = (parts: string[], description: Description) => {
  const flags = description.flags as FlagsType | undefined
  if (flags?.match !== undefined) {
    parts.push(`.match(${JSON.stringify(flags.match)})`)
  }
}
const appendMultiple = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'multiple')
  if (rule !== undefined) {
...
const appendNand = (parts: string[], description: Description) => {
  const dependency = (((description as any).dependencies ?? []) as DependencyType[]).find((it) => it.rel === 'nand')
  if (dependency !== undefined) {
    parts.push(`.nand(${dependency.peers.map((it) => JSON.stringify(it)).join(',')})`)
  }
}
...
const appendNote = (parts: string[], description: Description) => {
  ...
  if (values !== undefined) {
    parts.push(`.note(${values.map((value) => JSON.stringify(value)).join(',')})`)
  }
}

const appendNormalize = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'normalize')
  if (rule !== undefined) {
...
const appendOr = (parts: string[], description: Description) => {
  const dependency = (((description as any).dependencies ?? []) as DependencyType[]).find((it) => it.rel === 'or')
  if (dependency !== undefined) {
    parts.push(`.or(${dependency.peers.map((it) => JSON.stringify(it)).join(',')})`)
  }
}
...
const appendOrdered = (parts: string[], description: Description) => {
  ...
  if (items !== undefined) {
    parts.push(`.ordered(${items.map((it) => descriptionToString(it)).join(',')})`)
  }
}

const appendOxor = (parts: string[], description: Description) => {
  const dependency = (((description as any).dependencies ?? []) as DependencyType[]).find((it) => it.rel === 'oxor')
  if (dependency !== undefined) {
    parts.push(`.oxor(${dependency.peers.map((it) => JSON.stringify(it)).join(',')})`)
  }
}

const appendPattern = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'pattern')
  if (rule !== undefined) {
    parts.push(`.pattern(${rule.args.regex},${JSON.stringify(rule.args.options ?? {})})`)
  }
}

const appendPort = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'port')
  if (rule !== undefined) {
    parts.push('.port()')
  }
}
...
const appendPrecision = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'precision')
  if (rule !== undefined) {
...
const appendRegex = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'regex')
  if (rule !== undefined) {
    parts.push('.regex()')
  }
}
...
const appendReplace = (parts: string[], description: Description) => {
  const values = (description as any).replacements as { pattern: { regex: string }; replacement: string }[] | undefined
  if (values !== undefined) {
    parts.push(`.replace(${values[0].pattern.regex}, ${JSON.stringify(values[0].replacement)})`)
  }
}
...
const appendSign = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'sign')
...
const appendStringLength = (parts: string[], description: Description) => {
  ...
  if (rule !== undefined) {
    ...
    parts.push(
      `.length(${[JSON.stringify(rule.args?.limit), encoding !== undefined ? JSON.stringify(encoding) : undefined]
        .filter((it) => !!it)
        .join(',')})`
    )
  }
...
const appendStringMax = (parts: string[], description: Description) => {
  ...
  if (rule !== undefined) {
    const { encoding } = rule.args
    parts.push(
      `.max(${[JSON.stringify(rule.args?.limit), encoding !== undefined ? JSON.stringify(encoding) : undefined]
        .filter((it) => !!it)
        .join(',')})`
    )
  }
...
const appendStringMin = (parts: string[], description: Description) => {
  ...
  if (rule !== undefined) {
    const { encoding } = rule.args
    parts.push(
      `.min(${[JSON.stringify(rule.args?.limit), encoding !== undefined ? JSON.stringify(encoding) : undefined]
        .filter((it) => !!it)
        .join(',')})`
    )
  }
...
const appendToken = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'token')
  if (rule !== undefined) {
    parts.push('.token()')
  }
}
...
const appendTrim = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'trim')
  if (rule !== undefined) {
...
const appendTruthy = (parts: string[], description: Description) => {
  ...
  if (values !== undefined) {
    parts.push(`.truthy(${values.map((value) => JSON.stringify(value)).join(',')})`)
  }
}

const appendTry = (parts: string[], description: Description) => {
  const matches = (description as any).matches as { schema: Description }[] | undefined
  if (matches !== undefined) {
    parts.push(`.try(${matches.map((it) => descriptionToString(it.schema)).join(',')})`)
  }
}

const appendUnique = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'unique')
  if (rule !== undefined) {
    parts.push(`.unique(${JSON.stringify(rule.args?.comparator)},${JSON.stringify(rule.args?.options)})`)
  }
}
...
const appendUri = (parts: string[], description: Description) => {
  const rule = (description.rules as RuleType[] | undefined)?.find((it) => it.name === 'uri')
  if (rule !== undefined) {
...
const appendValid = (parts: string[], description: Description) => {
  ...
  if (values !== undefined) {
    parts.push(`.valid(${values.map((value) => JSON.stringify(value)).join(',')})`)
  }
}

const appendXor = (parts: string[], description: Description) => {
  const dependency = (((description as any).dependencies ?? []) as DependencyType[]).find((it) => it.rel === 'xor')
  if (dependency !== undefined) {
    parts.push(`.xor(${dependency.peers.map((it) => JSON.stringify(it)).join(',')})`)
  }
}
...
const alternativesDescriptionToString = (description: Description) => {
  ...
  appendCommon(parts, description)
  appendMatch(parts, description)
  appendTry(parts, description)
...
const objectDescriptionToString = (description: Description) => {
  ...
  parts.push(
    Object.entries(keys)
      .map(([key, value]) => `${JSON.stringify(key)}: ${descriptionToString(value)}`)
      .join(',')
  )
  parts.push('})')
  appendCommon(parts, description)
...
const addPresenceToDescription = (description: Description, presence: PresenceType) => {

...
const refineSchema = function (joiSchema: Joi.Schema, jsonSchema: OpenAPIV3.SchemaObject) {
...
const getOperationSchemas = (document: OpenAPIV3.Document) => {
  const allOperations = (Object.values(document.paths) as Record<string, OpenAPIV3.OperationObject>[])
    .filter((it) => !!it)
    .reduce<OpenAPIV3.OperationObject[]>(
      (arr, path) => [
        ...arr,
        ...Object.entries(path)
          .filter(([key, value]) => httpMethods.includes(key) && !!value.operationId && !!value.parameters?.length)
          .map(([, value]) => value),
      ],
      []
    )
  ...
  return allOperations
    .map((operation) => {
      ...
      const parameterSchemas = parameters
        .filter((it) => !!it.schema)
        .map((parameter) => {
          const joiSchema = enjoi.schema(parameter.schema! as OpenAPIV3.SchemaObject, { refineSchema })
          const parameterKey = JSON.stringify(parameter.name)
          const code = joiSchemaToCode(joiSchema, parameter.required === true ? 'required' : 'optional')
...
const getComponentSchemas = (document: OpenAPIV3.Document) =>
  Object.entries(document.components?.schemas ?? {})
    .map(([name, schema]) => {
      const joiSchema = enjoi.schema(schema as OpenAPIV3.SchemaObject, { refineSchema })
...
export default async (schemaPath: string, prettierConfigPath?: string) => {
  ...
  const mergedTemplate = TEMPLATE.replace('{OPERATION_SCHEMAS}', getOperationSchemas(document)).replace(
    '{COMPONENT_SCHEMAS}',
    getComponentSchemas(document)
  )

  const prettierOptions = (prettierConfigPath ? await prettier.resolveConfig(prettierConfigPath) : null) ?? {
    parser: 'typescript',
  }
